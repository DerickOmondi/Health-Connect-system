    ...
    const toggleBtn = document.querySelector('#toggleSignupPassword');
    const passwordInput = document.querySelector('#signup-password');
    const eyeIcon = document.querySelector('#eyeIcon');

    // SVG paths for "Open Eye" and "Eye with Slash"
    ...
    toggleBtn.addEventListener('click', function () {
        ...
        // Switch the SVG path to show the slash (hidden) or no slash (visible)
        eyeIcon.querySelector('path').setAttribute('d', isPassword ? closedEye : openEye);
    });